...
class ArchitectureBoundariesTest {
    ...
    @Test fun featureAndInfrastructureBoundariesExist() {
        listOf(
            ...
            "presentation/FeatureUiStates.kt",
            "navigation/FinanceAppNavigation.kt"
        ).forEach { relative -> assertTrue("Missing boundary: $relative", File(sourceRoot, relative).exists()) }
    }

    @Test fun featureScreensConsumeImmutableFeatureProjections() {
        val featureSource = File(sourceRoot, "feature").walkTopDown()
            .filter { it.extension == "kt" }
            .joinToString("\n") { it.readText() }
        assertTrue(featureSource.contains("dashboardUiState"))
        assertTrue(featureSource.contains("paymentsUiState"))
        assertTrue(featureSource.contains("expensesUiState"))
        assertTrue(featureSource.contains("reportsUiState"))
    }
