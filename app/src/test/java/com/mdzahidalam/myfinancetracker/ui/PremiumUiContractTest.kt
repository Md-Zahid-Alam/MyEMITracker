...
class PremiumUiContractTest {
    ...
    @Test fun largeTextAndFormsAdaptWithoutRemovingNavigationLabels() {
        ...
        assertTrue(dashboard.contains("rememberTextMeasurer"))
        assertTrue(dashboard.contains("amountNeedsFullWidth"))
        assertTrue(forms.contains("widthIn(max = FinanceLayout.formContentMax)"))
        assertTrue(navigation.contains("TextOverflow.Ellipsis"))
    }

    @Test fun signatureRemovalRequiresConfirmation() {
        val components = File(sourceRoot, "core/ui/FinanceComponents.kt").readText()
        assertTrue(components.contains("confirmRemoval = true"))
        assertTrue(components.contains("Remove signature?"))
        assertTrue(components.contains("Keep signature"))
    }
